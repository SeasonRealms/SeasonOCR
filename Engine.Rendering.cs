...
{
    private static byte[] DrawResults(ReadOnlySpan<byte> imageData, int width, int height, List<OcrBox> boxes)
    {
        var rgba = ImageProcessor.EnsureRgba(imageData, width, height);

        int thickness = Math.Max(2, Math.Min(width, height) / 400);

        foreach (var box in boxes)
        {
            if (box.Quad is { Length: 4 } && IsFreeFormQuad(box.Quad))
            {
                DrawQuadOutline(rgba, width, height, box.Quad, thickness, 0, 255, 0);
            }
            else
            {
                DrawRectOutline(rgba, width, height,
                    box.X, box.Y, box.Width, box.Height, thickness, 0, 255, 0);
            }
        }

        // 直接返回 RGBA 像素数据的只读跨度，不再编码为 JPEG
        return rgba;
    }
    ...
    private static byte[] BuildFallbackImage(ReadOnlySpan<byte> imageData, int width, int height)
    {
        var rgba = ImageProcessor.EnsureRgba(imageData, width, height);
        return rgba;
    }
